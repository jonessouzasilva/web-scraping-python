...
for hospedagem in hospedagens:
    ...
    print('Descriçao: ', hospedagem_descricao)
    print('URL: ', hospedagem_url)

    hospedagem_detalhes = hospedagem.find('div', attrs={'margin-bottom: 2px;'}).findAll('li')

    # hospedagem_detalhes = hospedagem_detalhes[0].text + hospedagem_detalhes[1].text
    hospedagem_detalhes = ''.join([detalhe for detalhe in hospedagem_detalhes])
    ...
    print = pd.DataFrame(dados_hospedagens, columns=['Descrição', 'URL', 'Detalhes', 'Preço'])
